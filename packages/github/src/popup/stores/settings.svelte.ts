...
import type { GitHubSearchType } from '@lib/url-builder';
...
export interface Settings {
  ...
  theme: Theme;
  defaultSearchType: GitHubSearchType;
...
const DEFAULTS: Settings = {
  ...
  theme: 'light',
  defaultSearchType: 'repositories',
...
const SEARCH_TYPES: ReadonlySet<GitHubSearchType> = new Set([
  'repositories',
  'code',
  'issues',
  'pullrequests',
 ...
 *  brick the popup. */
/**
 * Strict pre-import validation: rejects unsupported enum values so the
 * Save dialog surfaces a clear error before `clearAll()` wipes existing
 * storage. Complements the clamping done by `sanitizeSettings()` at
 * load time (which only kicks in after the bad data is already
 * persisted). Wired via `restoreImport(..., { validate: validateBackup })`.
 */
