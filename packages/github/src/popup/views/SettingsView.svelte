<script lang="ts">
  ...
  const PLUGIN_ID = 'github';
  const PLUGIN_NAME = 'GitHub Search Toolkit';
  ...
  const exportData = async () => {
    ...
    a.href = url;
    a.download = `github-search-toolkit-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
...
<div class="space-y-3 p-3">
  ...
  <div>
    ...
    />
    <p class="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
      Side panel stays open while you use GitHub. Popup is a quick-glance
  ...
  <div>
    ...
    <Select
      ...
      onChange={(v) =>
        settings.update({
          defaultSearchType: (v ?? 'repositories') as GitHubSearchType,
        })}
    />
  </div>
