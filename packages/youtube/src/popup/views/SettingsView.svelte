<script lang="ts">
  ...
  const PLUGIN_NAME = 'YouTube Search Toolkit';
  ...
  const exportData = async () => {
    ...
    a.href = url;
    a.download = `youtube-search-toolkit-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
...
<div class="space-y-3 p-3">
  ...
  <div>
    ...
    />
    <p class="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
      Side panel stays open while you use YouTube. Popup is a quick-glance
  ...
  <div>
    <div class="label">Default filter preset</div>
    <Select
      options={PRESET_OPTIONS}
      value={settings.value.defaultPreset}
      placeholder="No filter"
      onChange={(v) =>
        settings.update({ defaultPreset: (v ?? 'none') as YouTubeFilterPreset })}
    />
  </div>
