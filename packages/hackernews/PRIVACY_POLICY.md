# Hacker News Search Toolkit (Unofficial) — Privacy Policy

**Effective date:** 2026-05-11

Hacker News Search Toolkit ("the extension") is an **independent third-party (unofficial)** browser extension that helps you compose, save, and launch Hacker News search URLs
(via the Algolia HN search) from a side panel. This policy describes
what data the extension processes, where it is stored, and what
permissions it uses.
...
- **Saved searches** — names, tags, and the search-input shape you
  composed (keywords, points, comments, author, date range, etc.)
  plus the resulting hn.algolia.com search URL.
- **Tags** — labels you attach to saved searches for organization.
- **Settings** — theme, display mode (popup vs side panel), default
  landing view, open mode (current tab vs new tab).
