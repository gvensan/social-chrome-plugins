<script lang="ts">
  ...
  const PLUGIN_ID = 'hackernews';
  const PLUGIN_NAME = 'Hacker News Search Toolkit';
  ...
  const exportData = async () => {
    ...
    a.href = url;
    a.download = `hackernews-search-toolkit-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
...
<div class="space-y-3 p-3">
  ...
  <div>
    ...
    />
    <p class="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
      Side panel stays open while you browse HN. Popup is a quick-glance
