# Hugging Face Search Toolkit — Chrome Web Store Submission
...
> Unofficial. Compose Hugging Face Models / Datasets / Spaces search URLs with task, library, license filters.

### Description (required, 16,000 char max)

```
Hugging Face Search Toolkit (Unofficial) is a Chrome extension that
helps you compose, save, and launch Hugging Face search URLs from a
...
A guided builder for Hugging Face's three search facets — Models,
Datasets, and Spaces — each with the native filter set: pipeline-tag,
library, license, size category, modality, language, and (for Spaces)
SDK. A live URL preview updates as you compose, and every search can
be saved to your local library for one-click re-use.

═══ Use cases ═══

• Recent models on a topic: text-generation pipeline, sorted by
  recency, with a license filter — surface fresh releases without
  trawling the main Models index.

• Models trained on a dataset: filter Models by a specific dataset
  tag to find downstream work.

• Spaces using a model: filter Spaces by a model tag — see live demos
  built on a base model.

• Datasets for a task: filter Datasets by task tag (e.g. summarization,
  named-entity-recognition) with size-category and license bounds.

• Library-scoped model search: transformers + a specific language tag
  — narrow down to weights you can drop into an existing pipeline.
...
a Models/Datasets/Spaces facet search composer.)
...
Drop file: `packages/huggingface/public/icons/icon-128.png` (or
extract from `release/huggingface-v0.1.0.zip` → `icons/icon-128.png`).
...
Take all 5 after loading `packages/huggingface/dist/` as an unpacked
...
2. **Builder → Models** — with pipeline-tag, library, and license
   filters set and the live URL preview visible at the bottom.
3. **Builder → Datasets** — task / size-category / language filters
   showing the assembled `/datasets?...` URL.
...
```
Generate Hugging Face Models / Datasets / Spaces search URLs from a
guided form and launch them in the user's existing browser session.
```
...
Host the Hugging Face-specific policy (Skeleton A from
...
1. Update `packages/core/src/family.ts` — set `webStoreUrl` for the
   `huggingface` entry.
2. Bump `packages/huggingface/src/manifest.json` version (e.g.
   `0.1.1`).
3. Tag + push: `git tag huggingface-v0.1.1 && git push --tags`. CI
   builds the patch release and uploads it (with the new family URL
   in Hugging Face's own About view).
4. The OTHER 6 plugins still show `webStoreUrl: undefined` for
   Hugging Face — they'll show the live link only after their next
