<script lang="ts">
  ...
  const PLUGIN_ID = 'huggingface';
  const PLUGIN_NAME = 'Hugging Face Search Toolkit';
  const PLUGIN_VERSION = '0.1.0';
  import type { HFSearchType } from '@lib/url-builder';
  ...
  const exportData = async () => {
    ...
    a.href = url;
    a.download = `huggingface-search-toolkit-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
...
<div class="space-y-3 p-3">
  ...
  <div>
    ...
    />
    <p class="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
      Side panel stays open while you browse Hugging Face. Popup is a
      quick-glance window that closes on outside click.
    </p>
  </div>
  ...
  <div>
    ...
    />
    <p class="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
      Where Open buttons send the user. "Current tab" navigates the active
      tab in this window; "New tab" creates a fresh tab.
    </p>
  </div>
  ...
  <div>
    ...
    <Select
      ...
      onChange={(v) =>
        settings.update({
          defaultSearchType: (v ?? 'models') as HFSearchType,
        })}
    />
  </div>
